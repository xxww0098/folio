...
import { COMMAND_ITEMS, type ConsoleSection } from "./nav";
...
export function ConsoleCommand({
  ...
  const pages = useMemo(
    () => COMMAND_ITEMS.filter((item) => !q || `${item.label}${item.hint}`.toLowerCase().includes(q)),
    [q],
  );
  ...
  function goSection(section: ConsoleSection) {
    onOpenChange(false);
    void navigate({ to: "/console", search: { section } });
  }
